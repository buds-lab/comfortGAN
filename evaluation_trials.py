...
if sys.argv[1] == "-a":
    ...
    model_path = {"occutherm": config_occutherm['model_path'], "cresh": config_cresh['model_path']}
...
def sample_comfortgan(df_name, comfortgan, column_names):
    ...
    if df_name == "occutherm":
        occutherm_req_two = config_occutherm['occutherm_req_two']
        ...
        occutherm_req_minus_two =  config_occutherm['occutherm_req_minus_two']
        count_two = 0
        count_one = 0
        count_minus_one = 0
        count_minus_two = 0
        ...
        ashrae_req_three = config_ashrae['ashrae_req_three']
        ashrae_req_two = config_ashrae['ashrae_req_two']
        ...
        ashrae_req_minus_two = config_ashrae['ashrae_req_minus_two']
        ashrae_req_minus_three = config_ashrae['ashrae_req_minus_three']
...
#             print("All samples generated!")
        ...
        if df_name == "occutherm":
            for index, row in curr_df.iterrows():  
                if (row['Discrete Thermal Comfort_TA'] == 2.0) and (count_two != occutherm_req_two): 
                    df_synth = df_synth.append(row)
                    count_two += 1 
                elif (row['Discrete Thermal Comfort_TA'] == 1.0) and (count_one != occutherm_req_one): 
                    df_synth = df_synth.append(row)
                    count_one += 1
                elif (row['Discrete Thermal Comfort_TA'] == -1.0) and (count_minus_one != occutherm_req_minus_one): 
                    df_synth = df_synth.append(row)
                    count_minus_one += 1
                elif (row['Discrete Thermal Comfort_TA'] == -2.0) and (count_minus_two != occutherm_req_minus_two): 
                    df_synth = df_synth.append(row)
                    count_minus_two += 1
...
for df in datasets_train.keys():
    ...
    save_pickle(variability, "metrics/" + df + "-" + experiment_name + "_variability_comfortgan_trials.pkl")
    save_pickle(diversity, "metrics/" + df + "-" + experiment_name + "_diversity_comfortgan_trials.pkl")
    save_pickle(class_acc_test, "metrics/" + df + "-" + experiment_name + "_classification_test_comfortgan_trials.pkl")
    save_pickle(class_report_rdf_list, "label-metrics/" + df + "_class_report_" + experiment_name + "_comfortgan_trials.pkl")
